...
-- Nome do aluno: Marcos Vinicius De Souza Pereira
...
CREATE DATABASE LOJAMARCOSCJ3025349;
GO


-- Habilita o contexto do banco de dados
USE LOJAMARCOSCJ3025349;
GO
...
-- Utilize aliases para exibir o nome das colunas.(0.5 ponto)
	...
	WHERE
		TABLE_SCHEMA = 'LOJAMARCOSCJ3025349';
    GO
...
SELECT
    MAX(ValorUnitario) AS 'Maior_Preco',
...
SELECT
    ...
    CodigoProduto AS 'Codigo_Produto',
    Quantidade    AS 'Quantidade_Item'
FROM
    ITEM_DE_PEDIDO
WHERE
    Quantidade = 35;
...
SELECT
    ...
    Salario        AS 'Salario_Vendedor'
FROM
    vendedores
WHERE
    NomeVendedor LIKE 'A%' OR NomeVendedor LIKE 'C%' OR NomeVendedor LIKE 'F%' OR NomeVendedor LIKE 'M%'
ORDER BY
    NomeVendedor;
GO
...
SELECT
    C.CodigoCliente    AS 'Codigo do Cliente',
    C.NomeCliente      AS 'Nome do Cliente',
    P.DescricaoProduto AS 'Produto Comprado',
    P.ValorUnitario    AS 'Valor Unitario do Produto',
    IP.Quantidade      AS 'Quantidade Comprada',
    (P.ValorUnitario * IP.Quantidade) AS 'Valor Total a Pagar',
    PD.PrazoEntrega                   AS 'Prazo de Entrega do Pedido',
    V.NomeVendedor                    AS 'Nome do Vendedor'
FROM
    CLIENTES AS C
JOIN
    PEDIDOS AS PD ON C.CodigoCliente = PD.CodigoCliente
JOIN
    ITEM_DE_PEDIDO AS IP ON PD.NumeroPedido = IP.NumeroPedido
JOIN
    PRODUTOS AS P ON IP.CodigoProduto = P.CodigoProduto
JOIN
    VENDEDORES AS V ON PD.CodigoVendedor = V.CodigoVendedor
ORDER BY
    "Nome do Cliente",
    "Produto Comprado",
    "Quantidade Comprada",
...
DROP DATABASE LOJAMARCOSCJ3025349;
GO
